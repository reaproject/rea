...
#if defined(NDEBUG)
#error "REA cannot be compiled without assertions."
#endif
...
map<COutPoint, int> mapStakeSpent; //for additional check stake
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        nSubsidy = 957000 * COIN;
    } else if (nHeight < Params().LAST_POW_BLOCK()  && nHeight > 0) {
		nSubsidy = 1 * COIN;
    } else if (nHeight < 17281 && nHeight >= Params().LAST_POW_BLOCK()) {
        nSubsidy = 1 * COIN;
    } else if (nHeight < 23041  && nHeight >= 17281) {
        nSubsidy = 1.25 * COIN;
    } else if (nHeight < 28801 && nHeight >= 23041) {
        nSubsidy = 1.5 * COIN;
    } else if (nHeight < 34561 && nHeight >= 28801) {
        nSubsidy = 1.75 * COIN;
    } else if (nHeight < 40321 && nHeight >= 34561) {
        nSubsidy = 2 * COIN;
    } else if (nHeight < 46081 && nHeight >= 40321) {
        nSubsidy = 2.25 * COIN;
    } else if (nHeight < 51841  && nHeight >= 46081) {
        nSubsidy = 2.5 * COIN;
    } else if (nHeight < 57601  && nHeight >= 51841) {
        nSubsidy = 2.75 * COIN;
    } else if (nHeight < 63361 && nHeight >= 57601) {
        nSubsidy = 3 * COIN;
    } else if (nHeight < 69121  && nHeight >= 63361) {
        nSubsidy = 3.25 * COIN;
    } else if (nHeight < 74881  && nHeight >= 69121) {
        nSubsidy = 3.5 * COIN;
    } else if (nHeight < 80641  && nHeight >= 74881) {
        nSubsidy = 3.75 * COIN;
    } else if (nHeight < 86401  && nHeight >= 80641) {
        nSubsidy = 4 * COIN;
    } else if (nHeight < 92161  && nHeight >= 86401) {
        nSubsidy = 4.25 * COIN;
    } else if (nHeight < 97921  && nHeight >= 92161) {
        nSubsidy = 4.5 * COIN;
    } else if (nHeight < 103681  && nHeight >= 97921) {
        nSubsidy = 4.75 * COIN;
    } else if (nHeight < 109441  && nHeight >= 103681) {
        nSubsidy = 5 * COIN;
    } else if (nHeight < 115201  && nHeight >= 109441) {
        nSubsidy = 4.5 * COIN;
    } else if (nHeight < 120961  && nHeight >= 115201) {
        nSubsidy = 4 * COIN;
    } else if (nHeight < 126721  && nHeight >= 120961) {
        nSubsidy = 3.5 * COIN;
    } else if (nHeight < 132481  && nHeight >= 126721) {
        nSubsidy = 3 * COIN;
    } else if (nHeight < 138241  && nHeight >= 132481) {
        nSubsidy = 2.625 * COIN;
    } else if (nHeight < 700801  && nHeight >= 138241) {
        nSubsidy = 2.625 * COIN;
    } else if ( nHeight >= 700801) {
        nSubsidy = 2.625 * COIN;
		for (int i=0; i<nHeight/700800; i++) {
			nSubsidy=nSubsidy-(nSubsidy*5)/100 + (nSubsidy-(nSubsidy*5)/100)*5/100;
		}	
    } else {
        nSubsidy = 0 * COIN;
    }
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
	...
	if (nHeight == 0) {
		...
		ret = 0;
 	} else if (nHeight < 132481  && nHeight >= Params().LAST_POW_BLOCK()) {
        ret = blockValue * 0.8;
    } else {
        ret = 0.76 * blockValue;
    }
...
CScript GetScriptPubKeyFromAddr(const char * Addr) {
	CScript mScriptPubKey;
	CBitcoinAddress Address;

	Address=CBitcoinAddress((char *)Addr);
	mScriptPubKey=GetScriptForDestination(Address.Get());
	return mScriptPubKey;
}
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp)
{
	...
	//add check for “Fake Stake” attacks before write block
	if (block.IsProofOfStake()) {
		LOCK(cs_main);
