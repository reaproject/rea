...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int64_t nFees, bool fProofOfStake)
{
	...
	CAmount devPayment=GetDevPayment(pindexPrev->nHeight, blockValue);
    ...
    if (hasPayment) {
        if (fProofOfStake) {
			...
			if (devPayment > 0) {
				...
				txNew.vout[i+1].scriptPubKey = devScriptPubKey;
				txNew.vout[i+1].nValue = devPayment;
			}

            //subtract mn and dev payments from the stake reward
    ...
    BOOST_FOREACH (CMasternodePayee& payee, vecPayments) {
        ...
        BOOST_FOREACH (CTxOut out, txNew.vout) {
            if (payee.scriptPubKey == out.scriptPubKey) {
                if(out.nValue + 0.00001*COIN >= requiredMasternodePayment)
                    found = true;
                else
