...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
	...
	CScript devScriptPubKey = GetScriptPubKeyFromAddr(devAddr);
    std::map<std::string, std::string> mapValue = wtx.mapValue;
	CAmount fullreward=0; 
            ...
            for (unsigned int i = 1; i < wtx.vout.size(); i++) {
                ...
                if (ExtractDestination(wtx.vout[i].scriptPubKey, outAddress)) {
                    if (IsMine(*wallet, outAddress)) {
						...
						if ( wtx.vout[i].scriptPubKey == devScriptPubKey ) {
							sub.type = TransactionRecord::DevReward;
							fullreward+=wtx.vout[i].nValue;
							sub.credit = fullreward;
						}
						else {
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::MNReward || type == TransactionRecord::DevReward) {
