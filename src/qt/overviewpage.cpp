...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeReaAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeReaAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeReaAmount = strAnonymizeReaAmount.remove(strAnonymizeReaAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeReaAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeReaAmount * COIN) nMaxToAnonymize = nAnonymizeReaAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeReaAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeReaAmount));
        strAnonymizeReaAmount = strAnonymizeReaAmount.remove(strAnonymizeReaAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeReaAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
